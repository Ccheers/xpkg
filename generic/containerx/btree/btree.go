...
import (
	"github.com/ccheers/xpkg/generic/cmpx"
)
...
func (t *Tree[K, V]) search(x *node[K, V], key K, height int) (V, bool) {
	...
	if height == 0 {
		// leaf node
		for j := 0; j < x.m; j++ {
			if cmpx.Compare(key, children[j].key, t.less) == 0 {
		...
		// internal node
		for j := 0; j < x.m; j++ {
			if x.m == j+1 || cmpx.Compare(key, children[j+1].key, t.less) < 0 {
...
func (t *Tree[K, V]) insert(h *node[K, V], key K, val V, height int, valid bool) *node[K, V] {
	...
	if height == 0 {
		// leaf node
		for j = 0; j < h.m; j++ {
			if cmpx.Compare(key, h.children[j].key, t.less) == 0 {
				h.children[j].val = val
				h.children[j].valid = valid
				return nil
			} else if cmpx.Compare(key, h.children[j].key, t.less) < 0 {
				break
			}
		}
	} else {
		// internal node
		for j = 0; j < h.m; j++ {
			if (j+1 == h.m) || cmpx.Compare(key, h.children[j+1].key, t.less) < 0 {
